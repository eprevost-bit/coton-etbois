...
class PurchaseOrderCustom(models.Model):
    ...
    def _ensure_xml_id(self, record):
        """
        Busca si el registro ya tiene un ID Externo.
        Si NO tiene, LO CREA en la base de datos (ir.model.data).
        Esto es OBLIGATORIO para que la importación nativa funcione.
        """
        # 1. Buscar si ya existe
        domain = [('model', '=', record._name), ('res_id', '=', record.id)]
        xml_record = self.env['ir.model.data'].sudo().search(domain, limit=1)

        if xml_record:
            # Si existe, devolvemos el ID real (ej: __export__.purchase_order_35_xc82...)
            return f"{xml_record.module}.{xml_record.name}"

        # 2. Si no existe, LO REGISTRAMOS en la base de datos
        # Generamos un nombre único
        generated_name = f"{record._name.replace('.', '_')}_{record.id}_export"
        ...
        return f"__export__.{generated_name}"
